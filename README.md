...
在一组手写台账图片测试中，加入逐行视觉复核和相关后处理后，核心字段准确率有明显提升：

| 指标 | 修复前 LLM+VL | 修复后 LLM+VL | 变化 |
|---|---:|---:|---:|
| 匹配行数 | 94 | 96 | +2 |
| 姓名正确率 | 64.9% | 75.0% | +10.1 个百分点 |
| 身份证正确率 | 89.4% | 91.7% | +2.3 个百分点 |
| 金额正确率 | 91.5% | 90.6% | -0.9 个百分点 |
| 三项全对率 | 54.3% | 68.8% | +14.5 个百分点 |

可以看到，LLM_VL 对姓名和三项全对率提升最明显。金额字段本身更依赖列定位和数字边界判断，因此在个别样本上可能出现小幅波动，仍建议通过审核模式做最终确认。
